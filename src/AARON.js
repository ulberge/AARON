...
function AARON(p) {
  ...
  */
  ...
  function planning(state) {
    ...
    // Draw figure start
    if (debug) {
      p.stroke(0, 0, 155, 100);
      p.circle(state.figure.start.location.x, state.figure.start.location.y, 8);
    }
  ...
  function lines(state) {
    ...
    if (debug) {
      ...
      p.stroke(0, 0, 155, 100);
      p.circle(state.line.end.location.x, state.line.end.location.y, 8);
      const directionVec = state.line.end.location.clone().add(state.line.end.direction);
      p.line(state.line.end.location.x, state.line.end.location.y, directionVec.x, directionVec.y);
    }
  ...
  function sectors(state) {
    ...
    if (state.sectors && state.sectors.length > 0) {
      ...
      if (debug) {
        // Draw end point and end direction
        p.stroke(0, 0, 0, 200);
        p.circle(state.pen.location.x, state.pen.location.y, 2);
  ...
  function avoidance(state) {
    if (debug) {
      p.stroke(155, 0, 0);
      p.circle(state.pen.location.x, state.pen.location.y, 10);
      p.line(state.pen.location.x, state.pen.location.y, state.line.end.location.x, state.line.end.location.y);
    }
  }
